      ...
      <div className='bg-header-bot	text-white py-2 '>
        <div className='container flex justify-between items-center'>
          <div className='flex flex-row items-center'>
            <img onClick={()=>navigate('/')} className='h-8 px-2 cursor-pointer' src={img?img:''} alt="logo" />
          ...
          <div className='flex flex-row'>
            <BsSearch className='hidden md:block bg-header-top px-2 m-1 rounded-full h-8 w-8 cursor-pointer'/>
            {
              ...
              <img onClick={()=>Navigate()} className='cursor-pointer rounded-full w-8 h-8 object-cover' src={user?user.url:'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'} alt="profileImg" />
              ...
              >
                {user && <button onClick={()=>navigate('/userProfile')} className="block w-full text-left px-4 py-2 text-white">User Profile</button>}
                {vendor && <button onClick={()=>navigate('/vendorProfile')} className="block w-full text-left px-4 py-2 text-white">Vendor Profile</button>}
                {user && <button onClick={()=>navigate('/settings')} className="block w-full text-left px-4 py-2 text-white">Settings</button>}
