 ...
 const getMatchedUsers = async (id, matchModel, userModel, vendorModel) => {
    ...
    try {
      ...
      if(usersData.length===0){
        usersData = await vendorModel.find({ _id: { $in: matchIds } });
      }
   ...
   const isUserMatched = async (user1, user2, matchModel) => {
    try {
 ...
 const addMatch = async (user1, user2, matchModel) => {
